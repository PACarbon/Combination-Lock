...
//		SPI_LCD_Init();
		...
		OLED_Clear();
		OLED_ShowString(0,0,"put new password",16);
		...
		while(1)
		{
				...
				while(j--)				//三次机会
				{
					OLED_ShowString(0,0,"Enter password",16);
				...
				if(j!=-1)			//正确开锁
				{
						OLED_ShowString(0,0,"True",16);
															//开门
				}
				else
				{
					OLED_ShowString(0,0,"Warn!!!",16);
					PCout(13)=0;
				}
				key=0;
				FLAG=0;
