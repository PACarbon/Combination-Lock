...
void OLED_ShowNum(u8 x,u8 y,int num,u8 Length)
{
	...
	for (i = 0; i < Length; i++)							
	{
		OLED_ShowChar(x+8*i,y, num / OLED_Pow(10, Length - i - 1) % 10 + '0',16);
	}
}

void OLED_ShowSignedNum(u8 x,u8 y,int num,u8 Length)		//适用原点左右跳动的数据,长度不含符号
{
	uint8_t i;
	if(num<0)
	{
		...
		for (i = 0; i < Length; i++)							
		{
			OLED_ShowChar(x+8*i+8,y, num / OLED_Pow(10, Length - i - 1) % 10 + '0',16);
		}
	}
	else
	{
