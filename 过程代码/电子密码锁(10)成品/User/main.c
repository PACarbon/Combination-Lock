...
#define MAX_PASSWORD_LENGTH 12		//密码长度
...
void open_password(void)								//输入密码
{
		...
		while(attempts--)				//三次机会
		{
			...
			key=0;
			FLAG=0;
			if (arraysEqual(password, password_check_1, MAX_PASSWORD_LENGTH))	break;
		}
			if(attempts>0)			//正确开锁
			{
				shield=1;
...
void change_password(void)								//改密码
{
		...
		key=0;
		FLAG=0;
		if (arraysEqual(password, password_check_1, MAX_PASSWORD_LENGTH))							//输入新密码
		{
			LCD_Clear();
			...
			LCD_Clear();
			 ...
			 if (arraysEqual(password_check_1, password_check_2, MAX_PASSWORD_LENGTH))			//输入相同的新密码
